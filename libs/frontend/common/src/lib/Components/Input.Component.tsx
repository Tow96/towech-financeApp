...
export const Input = ({
  ...
  return (
    <div className="relative pb-2 mb-2 mt-7 w-full">
      <input
        type={type}
        placeholder=""
        {...register}
        id={id}
        className={`${inputVariableClasses} input-shadow peer box-border w-full rounded border-none bg-riverbed-800 px-2 py-1 text-lg font-normal focus:outline-none disabled:opacity-75 disabled:shadow-none ${className}`}
        {...props}
      />
      <label
        data-testid="label"
      ...
      <div className="h-4">
        {!!error && typeof error === 'string' && (
          <p className="text-xs text-cinnabar-300 pl-3 pt-1">{error}</p>
        )}
      </div>
    </div>
  );
